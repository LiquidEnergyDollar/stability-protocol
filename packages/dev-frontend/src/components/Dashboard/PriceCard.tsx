...
export const TournamentYieldCard = ({ variant = "mainCards" }: SystemStatsProps): JSX.Element => {
  const thresholdSelectorStores = useThresholdSelector(selector);
  const store = thresholdSelectorStores[0].store;
  const redemptionRate = store.piRedemptionRate;
  const annualizedRate = calculateTournamentYield(redemptionRate);

  return (
    <Card {...{ variant }} sx={{ display: ['none', 'block'], width: "100%" }}>
      <TopCard
        name={`${COIN} Yield Until Tournament End`}
        tooltip={TooltipText.tournamentYield(COIN)}
        imgSrc="./icons/scale-icon.png"
      >
...
function calculateTournamentYield(redemptionRate: Decimal): string {
  ...
  const ratePerYear = Number.parseFloat(redemptionRate.pow(secondsUntilEnd).toString());
...
function annualizeInterestRate(redemptionRate: Decimal): string {
  ...
  const ratePerYear = Number.parseFloat(redemptionRate.pow(secondsPerYear).toString());
  return ((ratePerYear * 100) - 100).toFixed(2);
}
